...
| Storify (borttagen) | - | - | - | Borttagen 2026-09-18. Routen `src/routes/dagars-avtryck/` (sidan, ej `checkin/`), endpointerna under `src/routes/api/storify/` och samtycket `diary_ai_storify` finns inte langre i koden. Ingen ersattare byggdes. Kvar i databasen: tabellen `storify_entries` och redan lamnade samtyckesrader - separat beslut om radering/export. |
