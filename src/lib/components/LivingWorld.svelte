...
<style>
	...
	.is-paused .world-effect { animation-play-state: paused !important; }
	...
	/* Den primära, tydligt läsbara vattenrörelsen: en varm ljusglimt som sveper
	   ...
	   bara högre opacitet på water-surface-mönstret ovan - måste komma efter
	   .world-water i källan för att override:a dess border/bakgrund korrekt. */
	.water-glint { border: 0; border-radius: 50%; background: radial-gradient(ellipse at center, rgba(255, 255, 255, 0.98) 0%, rgba(224, 245, 255, 0.65) 38%, transparent 72%); filter: blur(2px); mix-blend-mode: screen; opacity: 0; animation: waterGlintSweep var(--duration, 13000ms) ease-in-out var(--delay, 0ms) infinite; }
	...
	/* Kontinuerligt upprepade ringar på vattnet - samma waterRing-animation som
	   den slumpmässiga shore-ripple-händelsen ovan, men loopad (infinite i
	   stället för both) så vattnet faktiskt märks röra sig hela tiden, inte
	   bara ibland. Det här är den tydligaste "vatten rör sig"-signalen. */
	.water-ripple-loop { border-radius: 50%; border: 1px solid rgba(255, 255, 255, 0.5); background: radial-gradient(circle at center, rgba(255, 255, 255, 0.55), transparent 68%); filter: blur(0.3px); transform: translate3d(-50%, -50%, 0) scale(calc(var(--scale, 1) * 0.64)); animation: waterRing var(--duration, 4800ms) ease-out var(--delay, 0ms) infinite; }
	...
	@keyframes driftFloat { 0% { opacity: 0; transform: translate3d(0, 8%, 0) scale(0.8); } 12% { opacity: var(--opacity, 0.5); } 50% { transform: translate3d(3%, -10%, 0) scale(1.08); opacity: calc(var(--opacity, 0.5) * 0.8); } 88% { opacity: var(--opacity, 0.5); } 100% { opacity: 0; transform: translate3d(-2.5%, -22%, 0) scale(0.85); } }
	...
	@media (prefers-reduced-motion: reduce) { .world-effect { animation: none !important; transform: none !important; } .world-water, .world-bird, .world-butterfly, .world-leaf, .water-glint, .world-drift, .water-ripple-loop { opacity: 0 !important; } .world-light, .world-mist, .world-cloud, .world-foliage { opacity: calc(var(--opacity, 0.12) * 0.5); } .water-surface { opacity: calc(var(--opacity, 0.14) * 0.35) !important; } }
</style>
