	...
	<div class="evening-experience">
		...
		<div class="sleep-focus" aria-hidden="true"></div>
			...
			<!-- Samma dagliga fråga som tidigare bara gick att svara på från Mitt Hem
			     (gamla /dashboard). Kortet ligger under hela introduktionens första rad,
			     aldrig som en modal ovanpå scenen, och finns bara i DOM:en den dag
			     frågan är obesvarad, precis som förut. -->
			{#if dailyQuestion.show && dailyQuestion.question}
				<div class="evening-intro-daily">
					<CompanionDailyCard
			...
			{:else if dailyQuestion.reaction}
				<p class="daily-question-reaction evening-intro-daily" role="status" aria-live="polite">{dailyQuestion.reaction}</p>
			{/if}
		</div>
