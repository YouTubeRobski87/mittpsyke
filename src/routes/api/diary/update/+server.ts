...
function errorResponse(message: string, status: number) {
	const body: UpdateDiaryErrorResponse = { success: false, error: message };
...
function validateBody(
	input: unknown
): { ok: true; data: UpdateDiaryRequestBody } | { ok: false; error: string } {
	if (!input || typeof input !== 'object' || Array.isArray(input)) {
		return { ok: false, error: 'Invalid request body.' };
	}

	const body = input as Partial<UpdateDiaryRequestBody>;
...
export const PUT: RequestHandler = async ({ request }) => {
	...
	const response: UpdateDiarySuccessResponse = {
		success: true,
		diary: updated as DiaryRecord
	};
