...
async function requireAdmin(locals: App.Locals) {
	...
	if (!user?.is_super_admin) throw redirect(303, '/');
}

async function ensureAdmin(locals: App.Locals) {
	const user = await locals.getSession();
	if (!user?.is_super_admin) return fail(403, { error: 'Åtkomst nekad.' });
	return user;
}
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const [findingsResult, sourcesResult, runsResult] = await Promise.all([
		admin
			.from('radar_findings')
			.select('id, title, url, provider, published_at, category, summary, why_it_matters, recommended_action, relevance_score, confidence_score, status, source_is_official')
		...
		admin
			.from('radar_runs')
			.select('id, started_at, finished_at, status, sources_checked, candidates_found, findings_saved, error_message')
...
export const actions: Actions = {
	setStatus: async ({ locals, request }) => {
		...
		if ('error' in adminUser) return adminUser;
		...
		const { error } = await getAdminClient(locals).from('radar_findings').update({ status }).eq('id', id);
		if (error) {
