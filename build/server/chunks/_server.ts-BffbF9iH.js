...
const POST = async ({ request }) => {
  ...
  try {
    ...
    if (conversationId) {
      const { data: existingConversation, error: existingConversationError } = await supabase.from("conversations").select("id, user_id, category").eq("id", conversationId).eq("user_id", user.id).maybeSingle();
      ...
      let { data: createdConversation, error: createConversationError } = await supabase.from("conversations").insert({
        user_id: user.id,
        category,
        title
      }).select("id").single();
      ...
      if (missingTitleColumn) {
        const retry = await supabase.from("conversations").insert({
          user_id: user.id,
          category
        }).select("id").single();
    ...
    const { data: previousMessages, error: previousMessagesError } = await supabase.from("messages").select("role, content").eq("conversation_id", conversationId).order("created_at", { ascending: true }).limit(20);
...
export { POST };
//# sourceMappingURL=_server.ts-BffbF9iH.js.map
